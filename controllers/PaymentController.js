...
//     // ✅ Find order and populate user to check if admin
...
//     // ✅ Test mode only works for admin users
//     const isAdmin = order.user && order.user.isAdmin === true;
//     const isTestMode = process.env.RAZORPAY_TEST_MODE === 'true' && isAdmin;

//     if (isTestMode) {
//       console.log(`⚠️ TEST MODE ACTIVE - Admin user: ${order.user.email}`);

//       // Mark order as paid (skip Razorpay verification)
...
//       // ✅ Send Telegram notification with TEST MODE tag
//       try {
...
// ⚠️ <b>TEST MODE - PAYMENT SUCCESSFUL</b> ⚠️
...
//     // ✅ NORMAL PRODUCTION FLOW for non-admin users
...
//     // ✅ Send Telegram notification (PRODUCTION)
//     try {
//       const itemsList = order.items.map((item, idx) =>
//         `${idx + 1}. ${item.product.itemname} x${item.quantity} - ₹${(item.price * item.quantity).toFixed(2)}`
//       ).join('\n');
...
export const verifyRazorpayPayment = async (req, res) => {
  try {
    ...
    // ✅ ✅ ✅ CHECK ADMIN TEST ORDER **FIRST** - BEFORE ANY VALIDATION ✅ ✅ ✅
    ...
    // ✅ ADMIN TEST MODE - Skip ALL Razorpay verification (including required fields check)
...
⚠️ <b>THIS IS A TEST ORDER - NO REAL PAYMENT PROCESSED</b>
    ...
    // ✅ NORMAL PRODUCTION FLOW - ONLY FOR NON-ADMIN OR REAL PAYMENTS
    console.log('💳 ============================================');
